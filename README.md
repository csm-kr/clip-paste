...
설정이 끝나면 평소 캡처 흐름 그대로, 마지막에 `Ctrl+Shift+V` 만 누르면 됩니다.

**기본 — 이미지 붙여넣기**
```
1. 캡처도구로 캡처            (이미지가 클립보드에 들어감, 보통 Ctrl+C)
2. Claude 프롬프트 클릭
3. Ctrl+Shift+V
   → AHK 말풍선: ① 핫키 발동 → ④ 전송 중 → ⑥ 전송 완료
   → 프롬프트에 자동으로:   /paste ⏎
   → Claude: "이미지를 읽었습니다 — <무엇이 보이는지 요약>"
```

**지시와 함께** — `/paste` 가 자동 입력된 뒤, 이어서 타이핑해도 되고, AHK 없이 직접 칠 수도 있어요:
```
/paste 이 화면 레이아웃 깨진 곳 찾아줘
/paste 이 에러 로그 원인 분석해줘
```

**텍스트는 그대로** — `Ctrl+V` 는 손대지 않았으니 텍스트 복사·붙여넣기는 평소와 동일합니다. 이미지만 `Ctrl+Shift+V`.

> `Ctrl+Shift+V` 누르면 ①~⑥ 말풍선으로 진행 단계가 보입니다. 멈춘 번호로 어디서 막혔는지 바로 알 수 있어요 (아래 문제 해결 표 참고).
